...
watch(() => route.params.id, async (newId) => {
  if (newId) {
    await n.setCountryFilter(Number(newId));
  }
}, { immediate: true });

// Get country name from first item
const countryName = computed(() => {
  if (items.value.length > 0 && items.value[0].countryName) {
    return items.value[0].countryName;
  }
  return 'País';
});
...
onMounted(async () => {
  if (route.params.id) {
    await n.setCountryFilter(Number(route.params.id));
  }
});
  ...
  <main class="container">
    <div class="content">
      ...
      <header class="page-header" v-if="n.total.value > 0">
        <div class="page-info">
          <h1 class="page-title">Noticias por país: {{ countryName }}</h1>
          <p class="page-count">
      ...
      <section class="grid">
        ...
        <template v-else>
          <EmptyState 
            message="No se encontraron noticias en este país"
            @retry="n.fetchList" 
