...
watch(() => route.params.q, async (newQuery) => {
  if (newQuery) {
    await n.setSearchFilter(decodeURIComponent(newQuery as string));
  }
}, { immediate: true });

// Get search query from route
const searchQuery = computed(() => {
  return route.params.q ? decodeURIComponent(route.params.q as string) : '';
});

onMounted(async () => {
  if (route.params.q) {
    await n.setSearchFilter(decodeURIComponent(route.params.q as string));
  }
});
  ...
  <main class="container">
    <div class="content">
      ...
      <header class="page-header" v-if="n.total.value > 0">
        <div class="page-info">
          <h1 class="page-title">Resultados de búsqueda: "{{ searchQuery }}"</h1>
          <p class="page-count">
